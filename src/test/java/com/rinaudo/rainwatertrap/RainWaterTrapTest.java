...
public class RainWaterTrapTest {
    ...
    public void calculateReturnZeroOnNullParams() {
        int areaToCheck = target.trap(null);
        assertEquals("Invalid rain area calculated", 0, areaToCheck);
    }

    @Test
    public void calculateReturnZeroOnEmptyParams() {
        int areaToCheck = target.trap(Collections.emptyList());
        assertEquals(0, areaToCheck);
    }

    @Test(expected = IllegalArgumentException.class)
    public void calculateThrowAnExceptionOnNegativeValues() {
        target.trap(Arrays.asList(1, -2));
    }

    @Test(expected = IllegalArgumentException.class)
    public void calculateThrowAnExceptionOnHighto5Values() {
        target.trap(Arrays.asList(1, 6));
    }

    @Test
    public void simpleOperationReturnsOneAreaValueCorrectly() {
        assertEquals(1,
            target.trap(Arrays.asList(1, 0, 1)));
        assertEquals(1,
            target.trap(Arrays.asList(1, 0, 2)));
        assertEquals(1,
            target.trap(Arrays.asList(0, 2, 0, 1)));
    }

    @Test
    public void bulkOperationCheckReturnsZeroValueCorrectly() {
        assertEquals(0,
            target.trap(Arrays.asList(0, 0, 1, 1, 1)));
        assertEquals(0,
            target.trap(Arrays.asList(1, 1, 1, 0, 0)));
        assertEquals(0,
            target.trap(Arrays.asList(1, 1, 1, 1, 1)));
        assertEquals(0,
            target.trap(Arrays.asList(0, 0, 0, 0, 0)));
    }

    @Test
    public void calculateReturnAreaOnParams() {
        int areaToCheck = target.trap(Arrays.asList(0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1));
